...
try:
    from sindex import RTreeError, SpatialIndex
    HAS_SINDEX = True
except ImportError:
...
def _geo_op(this, other, op):
    ...
    if isinstance(other, GeoPandasBase):
        ...
        return gdf.GeoSeries([getattr(this_elem, op)(other_elem)
                             for this_elem, other_elem in zip(this, other)],
                             index=this.index, crs=crs)
    else:
        return gdf.GeoSeries([getattr(s, op)(other)
...
def _geo_unary_op(this, op):
    ...
    return gdf.GeoSeries([getattr(geom, op) for geom in this.geometry],
                     index=this.index, crs=this.crs)
...
class GeoDfBase(object):
    ...
    def representative_point(self):
        ...
        return gdf.GeoSeries([geom.representative_point()
    ...
    def buffer(self, distance, resolution=16):
        return gdf.GeoSeries([geom.buffer(distance, resolution) 
                             for geom in self.geometry],
                         index=self.index, crs=self.crs)

    def simplify(self, *args, **kwargs):
        return gdf.GeoSeries([geom.simplify(*args, **kwargs)
                             for geom in self.geometry],
                      index=self.index, crs=self.crs)
    ...
    def interpolate(self, distance, normalized=False):
        ...
        return gdf.GeoSeries([s.interpolate(distance, normalized) 
    ...
    def translate(self, xoff=0.0, yoff=0.0, zoff=0.0):
        ...
        return gdf.GeoSeries([affinity.translate(s, xoff, yoff, zoff) 
                             for s in self.geometry], 
            index=self.index, crs=self.crs)
    ...
    def rotate(self, angle, origin='center', use_radians=False):
        ...
        return gdf.GeoSeries([affinity.rotate(s, angle, origin=origin, 
    ...
    def scale(self, xfact=1.0, yfact=1.0, zfact=1.0, origin='center'):
        ...
        return gdf.GeoSeries([affinity.scale(s, xfact, yfact, zfact, 
    ...
    def skew(self, xs=0.0, ys=0.0, origin='center', use_radians=False):
        ...
        return gdf.GeoSeries([affinity.skew(s, xs, ys, origin=origin, 
            use_radians=use_radians) for s in self.geometry],
            index=self.index, crs=self.crs)
    ...
    def explode(self):
        ...
        return gdf.GeoSeries(geometries,
